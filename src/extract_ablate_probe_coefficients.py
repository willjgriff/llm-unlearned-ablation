    ...
    for result_entry in input_record.get("results", []):
        if "model_answers" in result_entry:
            for answer_entry in result_entry["model_answers"]:
                ...
                if not any(coefficients_match(coefficient, existing) for existing in discovered_coefficients):
...
def build_output_record(input_record, requested_coefficients, collapsed_results):
    ...
    """
    output_record = {
        key: value for key, value in input_record.items() if key not in {"results", "steering_coefficient", "steering_coefficients"}
    }
